...
function LatenessList() {
  ...
  const getLessonRegister = useQuery(
    ...
    async () => {
      return axios.get(
        "http://localhost:8000/api/getLessonRegister/" + lessonId + "/late"
      );
    },
    {
  ...
  const getJust = useQuery(
    ...
    async () => {
      return axios.get(
        "http://localhost:8000/api/showJust/" + lessonId + "/late"
      );
    },
    {
      keepPreviousData: false,
  ...
  const refuseJust = useQuery(
    ...
    async () => {
      return axios.post("http://localhost:8000/api/refuseJust/", {
        ...
        content: `your child  ${props.item.firstname} ${props.item.lastname} justification is refused please check register lateness `,
  ...
  const acceptJust = useQuery(
    ...
    async () => {
      return axios.post("http://localhost:8000/api/acceptJust/", {
        ...
        time: new Date().toLocaleTimeString().slice(0, 8),
        date: new Date().toISOString().split("T")[0],
        content: `your child  ${props.item.firstname} ${props.item.lastname} justification is accepted please check register lateness `,
        user_id: props.item.user_id,
        lessonId: props.item.lessons_id,
        id: props.item.student_id,
      });
      ...
      setSuccess(
        ...
        >
          <Alert variant="filled" severity="warning" sx={{ width: "100%" }}>
            ...
            The lateness is Justify you can't notify the parent—{" "}
  ...
  const addNotificationstore = (params) => {
    axios
      .post("http://localhost:8000/api/addNotificationstore", {
        type: "lateness Notification",
        time: new Date().toLocaleTimeString().slice(0, 8),
        date: new Date().toISOString().split("T")[0],
        content: `your child ${params.row.firstname} ${params.row.lastname} is lateness in the lesson of ${params.row.nameSubject} in ${params.row.date} at ${params.row.startTime} please check register lateness and add justification`,
