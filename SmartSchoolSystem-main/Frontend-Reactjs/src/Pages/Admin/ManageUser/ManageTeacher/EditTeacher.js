...
function EditTeacher() {
  ...
  const { isLoading, data,refetch } = useGetUser(teacherId);
  if (data) {
    if (data.data == "fail") {
      navigate(-2);
    }
  ...
  return (
    ...
    >
      <Box sx={{ mt: 2, ml: 2 }}>
        <IconBreadcrumbs teacherId={teacherId} />
      </Box>
      <Grid container mt={2}>
        <Grid item xs={12}>
  ...
  return (
    ...
    >
      ...
      <Link
        ...
        component={RouterLink}
        to={`/admin/users/teachers/EditTeachers/${props.teacherId}`}
      >
        Edit teachers
      </Link>
    </Breadcrumbs>
