...
function EditSecretary() {
  ...
  const { isLoading, data,refetch } = useGetUser(secretaryId);
  if (data) {
    if (data.data == "fail") {
      navigate(-2);
    }
    ...
    function doSomthing() {
      if (data) {
        if (data.data != "fail") {
          ...
          setUsername({ value: data.data.username, valid: true });
          setEmail({ value: data.data.email, valid: true });
          setPhone({ value: data.data.phone, valid: true });
          setGender({ value: data.data.gender, valid: true });
          setBirthday({ value: data.data.birthday, valid: true });
          setPassword({ value: data.data.password, valid: true });
          setAddress({ value: data.data.address, valid: true });
          setRe_EnterPassword({ value: data.data.password, valid: true });
        }
      }
    }
    doSomthing();
  ...
  return (
    ...
    >
      <Box sx={{ mt: 2, ml: 2 }}>
        <IconBreadcrumbs secretaryId={secretaryId} />
      </Box>
      <Grid container mt={2}>
        <Grid item xs={12}>
          <MainCard sx={{ width: "100%", height: "100%" }}>
            <Grid container>
              <Grid item xs={12} mb={1} display="flex" justifyContent="center">
                <Typography variant="h3">Edit Secretary</Typography>
              </Grid>
  ...
  return (
    ...
    >
      ...
      <Link
        ...
        component={RouterLink}
        to={`/admin/users/secretaries/EditSecretary/${props.SecretaryId}`}
      >
        Edit Secretaries
      </Link>
    </Breadcrumbs>
