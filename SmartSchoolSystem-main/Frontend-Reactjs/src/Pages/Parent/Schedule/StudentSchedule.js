...
function StudentSchedule() {
  ...
  const getSchedulerClass = useQuery("getSchedulerClass", async () => {
    return axios.get("http://localhost:8000/api/getSchedulerClass/" + classId);
  });
  const [data, setData] = React.useState([]);
  React.useEffect(() => {
    setData(
      getSchedulerClass.data
        ? getSchedulerClass.data.data.map((item) => {
            return {
              id: item.id,
              ids: item.id,
              allDay: false,
